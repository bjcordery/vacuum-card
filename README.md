...
| `error_code_entity` | `string`  | Optional     | An entity_id within the `sensor` domain holding fault/error detail, appended to the status text (`Error — <value>`) while the vacuum is in the `error` state. |
